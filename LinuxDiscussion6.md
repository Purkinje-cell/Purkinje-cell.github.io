...
![](./img/scr_LinuxDiscussion6_20230624162333.png)

在使用这个存储库时，首先使用 `git clone` 命令将 GitHub 上的远程存储库克隆至本地。并使用 `git log` 命令查看提交记录。
![](./img/scr_LinuxDiscussion6_20230624162342.png)
...
![](./img/scr_LinuxDiscussion6_20230624162353.png)

可以发现 git 发现了我们对该文件的修改。使用 `git diff` 命令具体查看修改内容。
![](./img/scr_LinuxDiscussion6_20230624162403.png)
使用 `git add` 命令将修改暂存至缓冲区，然后使用 `git commit -m "xxx"` 命令提交更改。
![](./img/scr_LinuxDiscussion6_20230624162412.png)
...
![](./img/scr_LinuxDiscussion6_20230624162422.png)
使用 `git log` 命令查看提交，发现经过 pull 之后远程仓库和本地仓库的 main 分支已经合并。

![](./img/scr_LinuxDiscussion6_20230624162432.png)
此时再进行 `git push`，发现已经能够向远程仓库提交。


![](./img/scr_LinuxDiscussion6_20230624162450.png)
在 GitHub 上查看仓库，发现已经更新至最新一次提交。

![](./img/scr_LinuxDiscussion6_20230624162441.png)
